...
if(!isset($_SESSION['hid']))
{
  header('location:login.php');
	...
	if(isset($_SESSION['hid'])){
		$id=$_SESSION['hid'];
		$sql = "SELECT * FROM hospital_data WHERE Hospital_Id='$id'";
				...
				<div class="card">
					...
					<div class="card-body">
					   <form action="file/updateprofile.php" method="post">
						...
						<input type="text" name="hname" value="<?php echo $row['Hospital_Name']; ?>" class="form-control mb-3">
						<label class="text-muted font-weight-bold">Hospital Email</label>
						<input type="email" name="hemail" value="<?php echo $row['Hospital_Email']; ?>" class="form-control mb-3">
						<label class="text-muted font-weight-bold">Hospital Password</label>
						<input type="text" name="hpassword" value="<?php echo $row['Passwordd']; ?>" class="form-control mb-3">
						<label class="text-muted font-weight-bold">Hospital Phone Number</label>
						<input type="text" name="hphone" value="<?php echo $row['Hospital_Ph']; ?>" class="form-control mb-3">
						<label class="text-muted font-weight-bold">Hospital City</label>
						<input type="text" name="hcity" value="<?php echo $row['Hospital_City']; ?>" class="form-control mb-3">
